...
def test_reward_nb_ratings(scorer):
    ...
    assert np.isclose(reward, expected_reward, atol=1e-5), f"Expected {expected_reward}, but got {reward}"
...
def test_compute_score(scorer):
    ...
    assert all(scores >= 0) and all(scores <= 100), "Les scores doivent être entre 0 et 100"
...
def test_empirical_cdf(scorer):
    ...
    assert np.array_equal(sorted_data, expected_sorted_data), "Les données doivent être triées"
    assert np.allclose(cdf, expected_cdf, atol=1e-5), "La CDF empirique n'est pas correcte"
...
def test_optimize_weights(scorer):
    ...
    assert np.isclose(poids_note + poids_nb_reviews, 1.0, atol=1e-5), "La somme des poids doit être égale à 1"
